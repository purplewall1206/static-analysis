...
with open('ast-func-parser.sh', 'w') as f:
    for cmd in cmds:
        if cmd == '\n':
            continue        
        if cmd.find('.S') != -1:
        ...
        count = count + 1
        res = res + prefix
        res = res + ' -emit-llvm  -o ' + objectfile + '.bc ' + cmd[cmd.rfind(' '):] + '.c\n'
        res = res +  'opt -load ~/static-analysis/IRUsageAnalysis/build/libIRUsage.so -irusage ' + objectfile + '.bc\n'
...
# -Xclang -load -Xclang /home/wangzc/Documents/static-analysis/ASTFuncAnalysis/build/lib/libASTFuncPARSER.so -Xclang -plugin -Xclang ASTFuncparser
#
